...
int main(void)
{
    ...
    while (!WindowShouldClose())    // Detect window close button or ESC key
    {
        ...
        for (int i = 0; i < MAX_COLORS_COUNT; i++) DrawRLRectangleRec(colorsRecs[i], colors[i]);
        DrawRectangleLines(10, 10, 30, 30, LIGHTGRAY);

        if (colorMouseHover >= 0) DrawRLRectangleRec(colorsRecs[colorMouseHover], Fade(WHITE, 0.6f));

        DrawRectangleLinesEx((RLRectangle){ colorsRecs[colorSelected].x - 2, colorsRecs[colorSelected].y - 2,
        ...
        if (showSaveMessage)
        {
            ...
            RLDrawText("IMAGE SAVED:  my_amazing_texture_painting.png", 150, 180, 20, RAYWHITE);
        }
