...
{
    ImageDrawRLRectangleRec(dst, (RLRectangle){ (float)posX, (float)posY, (float)width, (float)height }, color);
}
...
// Draw rectangle within an image
void ImageDrawRLRectangleRec(Image *dst, RLRectangle rec, Color color)
{
    // Security check to avoid program crash
    if ((dst->data == NULL) || (dst->width == 0) || (dst->height == 0)) return;
